...
if __name__ == '__main__':
    ...
    data = rightDf.set_index(['class_test', 'name'])
    print(data)
    l_data = leftDf.set_index(['class_test', 'name'])
    print(l_data)
    allDf = leftDf.join(rightDf.set_index(['class_test', 'name']), on=['class_test', 'name'], how='right', lsuffix='_l', rsuffix='_r')
    print('xxxxxxxxxxxxxx')
    print(allDf)

    allDf2 = pd.merge(leftDf, rightDf, how='right', left_on=['class_test', 'name'], right_on=['class_test', 'name'], suffixes=('_x', '_y'))
    print(allDf2)
